...
AI로 만든 해당 코드는 항목별 정리해주는 내용으로 이해했습니다. 각각의 항목을 별개로 저장하는 것이 아닌 한개의 묶음으로 조회되도록 설정되어있었고,  struct라는 걸 써서 일정 하나를 통째로 묶어서 관리하는 게 핵심이에요.

프로그램이 켜지면 메뉴가 계속 떠 있는데, 내가 번호를 누를 때마다 정해진 기능으로 점프해서 실행돼요. 특히 신기했던 건 정렬 부분인데, 날짜가 빠른 순서대로 두 개씩 비교해서 자리를 바꾸는 식으로 줄을 세우더라고요. 마지막에 종료할 때 파일로 저장까지 하니까 프로그램을 껐다 켜도 내 일정이 그대
